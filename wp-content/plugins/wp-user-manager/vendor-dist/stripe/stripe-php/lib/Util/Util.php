...
namespace WPUM\Stripe\Util;

use WPUM\Stripe\StripeObject;
abstract class Util
{
    ...
    {
        $types = \WPUM\Stripe\Util\ObjectTypes::mapping;
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
        ...
        if (\is_array($resp)) {
            if (isset($resp['object']) && \is_string($resp['object']) && isset($types[$resp['object']])) {
                ...
                $class = \WPUM\Stripe\StripeObject::class;
            }
    ...
    {
        if ($h instanceof \WPUM\Stripe\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
