...
class FileService extends \WPUM\Stripe\Service\AbstractService
{
    /**
    ...
    {
        $opts = \WPUM\Stripe\Util\RequestOptions::parse($opts);
        if (!isset($opts->apiBase)) {
            $opts->apiBase = $this->getClient()->getFilesBase();
        }
        // Manually flatten params, otherwise curl's multipart encoder will
        // choke on nested null|arrays.
        $flatParams = \array_column(\WPUM\Stripe\Util\Util::flattenParams($params), 1, 0);
