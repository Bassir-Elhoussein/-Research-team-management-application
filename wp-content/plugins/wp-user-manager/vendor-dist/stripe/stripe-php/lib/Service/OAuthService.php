...
class OAuthService extends \WPUM\Stripe\Service\AbstractService
{
    /**
    ...
    public function authorizeUrl($params = null, $opts = null)
    {
        ...
        $query = \WPUM\Stripe\Util\Util::encodeParameters($params);
    ...
    private function _parseOpts($opts)
    {
        if (\is_array($opts)) {
            if (\array_key_exists('connect_base', $opts)) {
                ...
                throw new \WPUM\Stripe\Exception\InvalidArgumentException('Use `api_base`, not `connect_base`');
            }
        }
        return \WPUM\Stripe\Util\RequestOptions::parse($opts);
    }
    /**
